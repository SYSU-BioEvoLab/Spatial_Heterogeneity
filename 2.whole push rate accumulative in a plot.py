...
p_color = ['#6495ED', '#FF7F50', '#6e6ee5', '#5bd85b',
 ...
 # same color for boxplot and jaccard index
...
if __name__ == "__main__":
    ...
    for i, j, z in zip(all_VAF_f11, all_VAF_cumulative11, all_VAF_number11):
        # trendline(i, j, z, 11, alpha=0.6, c=p_color[1], Rval=True)
        plt.scatter(i, j, s=10,alpha=0.6, c=p_color[1])
    ...
    for i, j, z in zip(all_VAF_f12, all_VAF_cumulative12, all_VAF_number12):
        # trendline(i, j, z, 12, alpha=0.6, c=p_color[2], Rval=True)
        plt.scatter(i, j, s=10,alpha=0.6, c=p_color[2])



    plt.plot(VAF_f13, yvals13, 'r', )
    for i, j, z in zip(all_VAF_f13, all_VAF_cumulative13, all_VAF_number13):
        # trendline(i, j, z, 13,alpha=0.6, c=p_color[3], Rval=True)
        plt.scatter(i, j, s=10,alpha=0.6, c=p_color[3])


    plt.plot(VAF_f14, yvals14, 'r', )
    for i, j, z in zip(all_VAF_f14, all_VAF_cumulative14, all_VAF_number14):
        # trendline(i, j, z, 14, alpha=0.6, c=p_color[4],Rval=True)
        plt.scatter(i, j, s=10,alpha=0.6, c=p_color[4])
    ...
    for i, j, z in zip(all_VAF_f15, all_VAF_cumulative15, all_VAF_number15):
        # trendline(i, j, z, 15, alpha=0.1, c=p_color[5], Rval=True)
        plt.scatter(i, j, s=10,alpha=0.6, c=p_color[5])
    plt.xlabel('Inverse allelic frequency 1/f')

    plt.plot(VAF_f16, yvals16, 'r', )
    for i, j, z in zip(all_VAF_f16, all_VAF_cumulative16, all_VAF_number16):
        # trendline(i, j, z, 16, c=p_color[6],alpha=0.1, Rval=True)
        plt.scatter(i, j, s=10,alpha=0.6, c=p_color[6])
    plt.xlabel('Inverse allelic frequency 1/f')
    plt.xlim(0,500)
    plt.ylim(0,5000)
    ...
    pdf = matplotlib.backends.backend_pdf.PdfPages('whole+{}.pdf'.format(random.randint(1,100)) )
