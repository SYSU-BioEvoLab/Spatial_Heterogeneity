'''
2.this part plot the surface growth and pushrate growth in fig 1
  [Cancer growth plot]
  [2021/6/15]
...
Max_ROW = 150
Max_COL = 150
push_rate = 0.1  # the frequency of cancer cell push each others
...
if __name__ == "__main__":
    ...
    map = matplotlib.colors.ListedColormap(
        ['#FFFFFF', '#00008B', '#FF0000', '#FFFFFF', '#0000FF', '#6495ED', '#FF7F50', '#ff8d00', '#006400',
         ...
         '#9ACD32', '#60D898', '#008B8B', '#9400D3', '#00BFFF', '#DB6E6E', ])
    ...
    clist = ['white','red',"#F4BBB9",'yellow', "#DEE291","#CEA8EF",'#B354F7' ]

    newmap = LinearSegmentedColormap.from_list('chaos', clist)

    plt.figure('whole heatmap', figsize=[6, 5])

    # cdict = ['#FFFFFF','#FF0000',  '#0000FF', '#6495ED', '#FF7F50', '#ff8d00', '#006400',
    #      '#8FBC8F', '#9400D3', '#00BFFF','#008B8B', '#EF6273','#E8E20A',
    #          '#EFABAB','#BA4EF9',]
    #
    # cmaps = colors.ListedColormap(cdict)


    plt.title('Pushrate: %s  Cancer Number: {}'.format(cancer_num) % push_rate)
    sns.heatmap(data=cancer_matrix, yticklabels=False, xticklabels=False, cmap=newmap, )

    pdf = matplotlib.backends.backend_pdf.PdfPages('Growthplot %s-%s+{}.pdf'.format(cancer_num) % (push_rate,random.randint(1,100)))
    for fig in range(1,Max_generation+3):  ## will open an empty extra figure :(
        pdf.savefig(fig)
