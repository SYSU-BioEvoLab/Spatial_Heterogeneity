...
def trendline(xd, yd,zd, term, order=1, c='r', alpha=0.05, Rval=False):
...
def Func_fitting_plot(NP,N,X0,Y0):
    ...
    plt.scatter(X0, Y0, c=p_color[NP], s=10, alpha=1)
    plt.xlabel('Distance')
...
def Jacard_boxplot(NP,X_distance,Y_jaccard):  #plot th boxplot for diffrent dstance 20 -100
    ...
    fig = plt.figure('jacard_boxplot{}'.format(NP), figsize=[5, 5])
    ...
    Func_fitting_plot(2*NP,2, X0, Y0)
...
if __name__ == "__main__":
    ...
    for fig in range(1, 23):  ## will open an empty extra figure :(
        pdf.savefig(fig)
    pdf.close()
    plt.show()
    plt.ion()
