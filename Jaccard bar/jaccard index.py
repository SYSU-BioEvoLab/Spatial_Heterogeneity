...
pcolor = ['#6495ED', '#FF7F50', '#ff8d00', '#006400',
...
def trendline(xd, yd,zd, term, order=1, c='r', alpha=0.2, Rval=False):
    ...
    if not Rval:
        ...
        print('slope{}'.format(term),a, '$R^2 = %0.3f' % Rsqr,'K-S',KS)
...
def Jacard_boxplot( n,Y_jaccard,X_distance,):
    ...
    point4 = []
    point5 = []
    p_color = ['gold', 'k', 'g', 'b', 'm', 'c','#00BFFF', '#FFD700', '#CD5C5C', '#556B2F', '#9932CC', '#8FBC8F', '#2F4F4F',
         '#CD853F', '#FFC0CB', '#4169E1', '#FFFFE0', '#ADD8E6', '#008000', '#9400D3',
    ...
    bplot1 = ax.boxplot(all_point,vert=True,patch_artist = True, color= p_color, # vertical box alignment
                     labels=labels)
    for box in bplot1['boxes']:
        box.set(color=p_color[int(1-math.log((push_rate),2))],
                linewidth=1)
    ...
    plt.scatter(X_distance, Y_jaccard, c=p_color[int(1-math.log((n),2))],s=10, alpha=0.5)
    plt.xlabel('Distance')
    plt.ylabel("Jaccard index")
...
if __name__ == "__main__":
    ...
    pdf = matplotlib.backends.backend_pdf.PdfPages('0.1+{} %s.pdf'.format(cancer_num) % (push_rate,random.randint(1,100)))
    for fig in range(1, 12):  ## will open an empty extra figure :(
        pdf.savefig(fig)
