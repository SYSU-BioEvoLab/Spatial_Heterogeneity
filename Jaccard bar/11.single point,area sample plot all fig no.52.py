...
mutation_type4 = 30
mutation_type5 = 50
p_color = ['D8D03F',  '#FC8C62', '#8DA0CB', '#E78AC3', '#A6D854','#FFD92F', '#7ECECA', '#CD5C5C', '#556B2F', '#9932CC', '#8FBC8F', '#2F4F4F',
         '#CD853F', '#FFC0CB', '#469E1', '#FFFFE0', '#ADD8E6', '#008000', '#9400D3',
         '#9ACD32', '#D2B48C', '#008B8B']# same color for boxplot and jaccard index
pcolor = ['#F9E453', '#FF7F50', '#6e6ee5',
...
f = open('%s+{}_slope-Rsqr-KS.csv'.format(Max_generation) % push_rate, 'a', encoding='utf - 8')
f.write('Number,')
f.write('Slope,  ')
f.write('R^2,  ')
f.write('Ks\n')
...
def trendline(xd, yd,zd, term, order=1, c='r', alpha=0.01, Rval=False):
...
def Distance_nuetral_KS(xd, yd):   #Kolmogorov–Smirnov test with neutral line
...
def Func_fitting_plot(NP,N,X0,Y0):
    plt.figure('%s-jacard_index{}'.format(NP) % N, figsize=[5, 5])
    plt.scatter(X0, Y0, c=pcolor[NP],edgecolors=pcolor[NP], s=10, alpha=0.5)
    plt.xlabel('Distance')
    ...
    a = popt[0]
    coeffs = np.polyfit(X0, Y0, 1)

    # Calculate R Squared
    p = np.poly1d(coeffs)

    ybar = np.sum(Y0) / len(Y0)
    ssreg = np.sum((p(X0) - ybar) ** 2)
    sstot = np.sum((Y0 - ybar) ** 2)
    Rsqr = ssreg / sstot
    ...
    f = open('%s_%s_a+Rsqr+{}.csv'.format(N) % (push_rate,Max_generation), 'a', encoding='utf - 8')
    f.write('{},  '.format(NP))
    f.write('{},  '.format(a))
    f.write('{}\n  '.format(Rsqr))
    f.close()
...
def Jacard_boxplot(n,X_distance,Y_jaccard):  #plot th boxplot for diffrent dstance 20 -100
    ...
    # for i in Y_jaccard:
    #     if i < 0.01 :
    #         X1.append(y_jacard_dict[i])
    # for i in X_distance:
    #     if len(X1)==0:
    #         X0.append(i)
    #         Y0.append(jacard_dict[i])
    #     elif i < min(X1):
    #         X0.append(i)
    #         Y0.append(jacard_dict[i])

    for i in range(len(Y_jaccard)):
        if Y_jaccard[i] > 0.05 :
            X1.append(X_distance[i])
    for i in range(len(Y_jaccard)):
        if X_distance[i] < max(X1):
            X0.append(X_distance[i])
            Y0.append(Y_jaccard[i])
    ...
    sns.boxplot(data=all_point, color=pcolor[n], linewidth=0.5)
    plt.xlabel('Distance')
    plt.xticks(range(0, 5), labels)
    plt.ylabel("Jaccard index")

    plt.figure('jacard_poin all', figsize=[5, 5])
    sns.swarmplot(data=all_point, color=pcolor[n], edgecolor='k', linewidth=0.03, alpha=0.6)
    plt.xlabel('Distance')
    plt.ylabel("Jaccard index")
    plt.xticks(range(0, 5), labels)
    ...
    sns.violinplot(data=all_point, notch=True, vert=True, patch_artist=True, alpha=0.5,  # vertical box alignment
                   xlabels=labels, color=pcolor[n], linewidth=0.4, inner=None)
    sns.swarmplot(data=all_point, color=pcolor[n], edgecolor='white', linewidths=0.1, dodge=False, size=3,
                  alpha=0.8)
    plt.xlabel('Distance')
    plt.ylabel("Jaccard index")
    plt.xticks(range(0, 5), labels)

    plt.figure('jacard_Violin_all22', figsize=[5, 5])
    sns.violinplot(data=all_point, notch=True, alpha=0.5, vert=True, patch_artist=True,  # vertical box alignment
                   xlabels=labels, color=pcolor[n], linewidth=0.4)
    ...
    sns.violinplot(data=all_point, notch=True, vert=True, patch_artist=True,  # vertical box alignment
                   xlabels=labels, color=pcolor[n], linewidth=0.5)
    sns.swarmplot(data=all_point, color="white", alpha=0.8, dodge=True, size=3)
    plt.xlabel('Distance')
    plt.ylabel("Jaccard index")
    plt.xticks(range(0, 5), labels)

    plt.figure('jacard_SNS%s' % n, figsize=[5, 5])
    sns.boxplot(data=all_point, color=pcolor[n], linewidth=0.2, width=0.75)
    ...
    plt.scatter(X_distance, Y_jaccard, c=pcolor[n], s=30, edgecolors='k', linewidths=0.001, alpha=0.5)
    plt.xlabel('Distance')
    plt.ylabel("Jaccard index")
    ...
    plt.scatter(X_distance, Y_jaccard, c=pcolor[n], s=20, edgecolors='k', linewidths=0.001, alpha=0.5)
    plt.xlabel('Distance')
    plt.ylabel("Jaccard index")
    plt.legend(labels='', loc='upper right', shadow=bool, title='Jaccard Num= {}'.format(len(Y_jaccard)),
               numpoints=None)

    plt.figure('jacard_index_cut21', figsize=[5, 5])
    plt.scatter(X_distance, Y_jaccard, c=pcolor[n], s=20, edgecolors='k', linewidths=0.00, alpha=0.5)
    plt.xlabel('Distance')
    plt.ylabel("Jaccard index")
    plt.legend(labels='', loc='upper right', shadow=bool, title='Jaccard Num= {}'.format(len(Y_jaccard)),
               numpoints=None)
    ...
    plt.scatter(X_distance, Y_jaccard, c=pcolor[n], s=10, edgecolors='k', linewidths=0.001, alpha=0.5)
    plt.xlabel('Distance')
    plt.ylabel("Jaccard index")
    plt.legend(labels='', loc='upper right', shadow=bool, title='Jaccard Num= {}'.format(len(Y_jaccard)),
               numpoints=None)
    ...
    Func_fitting_plot(n,1,X_distance,Y_jaccard)
    Func_fitting_plot(n,2, X0, Y0)


    # plt.figure('jacard_index whole', figsize=[5, 5])
    # plt.scatter(X_distance, Y_jaccard, c=pcolor[int(1 - math.log((push_rate), 2))], s=10, alpha=1)
    # plt.xlabel('Distance')
    # plt.ylabel("Jaccard index")
    ...
    # plt.scatter(X0, Y0, c=pcolor[int(1-math.log((push_rate),2))],s=10,alpha=1)
    # plt.xlabel('Distance')
    # plt.ylabel("Jaccard index")
    # plt.legend(labels='',loc='upper right', shadow=bool, title='Jaccard Num= {}'.format(len(Y0)), numpoints=None)
    # popt, pcov = curve_fit(jaccard_func, X0, Y0)
...
if __name__ == "__main__":
    ...
    for i_cut in range(0,7):
        for i_ra in range(1,sample_size+1):
            ...
            if i_cut == 0:
                ...
                XY_range0.append(XY_center)
                for cut in range(0, len(cell_r)):
                    x0 = cell_r[cut]
                    y0 = cell_c[cut]
                    if X_center - d11 <= x0 < X_center + d11 and Y_center - d11 <= y0 < Y_center + d11:
                        horizon_x0.append(x0)
                        horizon_y0.append(y0)

                    mutation_ID = cell_dictionary[x0, y0].mutation  # get all the mutation ID in the dictionary
                    all_mutation_ID += mutation_ID  # Collect every mutation.ID in the all_mutation_ID

                ### 0 slice
                for cut0 in range(0, len(horizon_x0)):
                    x0 = horizon_x0[cut0]
                    y0 = horizon_y0[cut0]
                    mutation_ID0 = cell_dictionary[x0, y0].mutation  # get all the mutation ID in the dictionary

                    all_mutation_ID0 += mutation_ID0  # Collect every mutation.ID in the all_mutation_ID1

                result0 = Counter(all_mutation_ID0)  # Count frequency of each mutation ID
                count_mu0 = []  # empty list: record the mutation ID in the VAF
                count_times0 = []  # empty list: record the frequency of mutation ID in the VAF
                # print("n01",  len(all_mutation_ID0))
                ID_key =[]

                for i in result0:

                    if result0[i] >= 2*cancer_num0/Filter_frequency:
                        count_mu0.append(i)
                        count_times0.append(result0[i])
                    else:
                        continue

                all_mutation_dict0[XY_center] = count_mu0
                # print("nu",len(count_mu0),len(all_mutation_ID0))
                # counter the mutation proberbility in the clip area
                VAF0 = list(map(lambda c: c / (2 * cancer_num0),
                                 count_times0))  # Object function change to list, and calculate each element of count_times

                result_VAF0 = Counter(VAF0)  # Count each element in VAF, feedback the numbers

                VAF_mu0 = []  # empty list: record the mutation ID in the VAF
                VAF_times0 = []  # empty list: record the each ID frequency in the VAF
                for i0 in result_VAF0:
                    VAF_mu0.append(i0)

                list.sort(VAF_mu0, reverse=1)
                # VAF_mu0 = VAF_mu0[0: (len(VAF_mu0) - 3)]  #remove last 3 point
                # VAF_mu0.remove(0.5)
                # VAF_mu0 = list(filter(lambda x: x >= Filter_frequency, VAF_mu0))
                for j0 in VAF_mu0:
                    VAF_times0.append(result_VAF0[j0])

                VAF_cumulative0 = []
                VAF_number0 = list(VAF_times0)
                for f0 in range(1, 1+len(VAF_number0)):
                    VAF_cumulative0.append(np.sum(VAF_number0[0:f0]))
                VAF_f0 = list(map(lambda c: 1 / c, VAF_mu0))
                yvals0 = list(map(lambda c: 10 * c-20, VAF_f0))

                all_VAF_f0.append(VAF_f0)
                all_VAF_number0.append(VAF_number0)
                all_VAF_cumulative0.append(VAF_cumulative0)
                ...
                # print('location',XY_range0,all_mutation_dict0)
                jacard_run0 = jacard_run(XY_range0,all_mutation_dict0)
    ...
    print("time:",run_time)
    plt.figure(('whole accumulative 01'), figsize=[5, 5], linewidth=1)
    for i, j, z in zip(all_VAF_f0, all_VAF_cumulative0, all_VAF_number0):
        plt.scatter(i, j, s=40, edgecolors='k', linewidths=0.001, alpha=0.6)



    plt.figure(('whole accumulative 0'), figsize=[5, 5], linewidth=1)
    for i, j, z in zip(all_VAF_f0, all_VAF_cumulative0, all_VAF_number0):
        trendline(i, j, z, 10, c='b', Rval=True)
        plt.scatter(i, j, s=40, edgecolors='k', linewidths=0.001, alpha=0.6)

        D_ks, W_Ks = Distance_nuetral_KS(i, j)
        f = open('%s+{}_KS.csv'.format(Max_generation) % push_rate, 'a', encoding='utf - 8')
        f.write('{},  '.format(0))
        f.write('{},  '.format(round(D_ks, 4)))
        f.write('{}\n  '.format(round(W_Ks, 4)))
        f.close()
    ...
    for i, j, z in zip(all_VAF_f11, all_VAF_cumulative11, all_VAF_number11):
        trendline(i, j, z, 11, c='b', Rval=True)
        plt.scatter(i, j, s=40,edgecolors='k',linewidths=0.001,alpha=0.6)
    ...
    plt.legend(labels='', loc='upper left', shadow=bool, title='Cancer Num= {}'.format(cancer_num11), numpoints=None)
    plt.plot(VAF_f11, yvals11, 'r', )
    plt.ylabel("Cumulative Number of Mutation")
    plt.subplot(232)
    ...
    for i, j, z in zip(all_VAF_f12, all_VAF_cumulative12, all_VAF_number12):
        ...
        f.write('{}\n  '.format(round(W_Ks, 4)))
        f.close()
    plt.legend(labels='', loc='upper left', shadow=bool, title='Cancer Num= {}'.format(cancer_num12), numpoints=None, )
    plt.subplot(233)
    plt.plot(VAF_f13, yvals13, 'r', )
    ...
    plt.legend(labels='', loc='upper left', shadow=bool, title='Cancer Num= {}'.format(cancer_num13), numpoints=None)
    plt.subplot(234)
    plt.plot(VAF_f14, yvals14, 'r', )
    ...
    plt.legend(labels='', loc='upper left', shadow=bool, title='Cancer Num= {}'.format(cancer_num14), numpoints=None)
    plt.ylabel("Cumulative Number of Mutation")
    ...
    plt.legend(labels='', loc='upper left', shadow=bool, title='Cancer Num= {}'.format(cancer_num15), numpoints=None)
    plt.xlabel('Inverse allelic frequency 1/f')
    plt.subplot(236)
    plt.plot(VAF_f16, yvals16, 'r', )
    ...
    plt.legend(labels='', loc='upper left', shadow=bool, title='Cancer Num= {}'.format(cancer_num16), numpoints=None)
    plt.xlabel('Inverse allelic frequency 1/f')
    ...
    D_ks, W_Ks = Distance_nuetral_KS(VAF_f, VAF_cumulative)
    f = open('%s+{}_KS.csv'.format(Max_generation) % push_rate, 'a', encoding='utf - 8')
    f.write('{},  '.format(0))
    f.write('{},  '.format(round(D_ks, 4)))
    f.write('{}\n  '.format(round(W_Ks, 4)))
    ...
    plt.scatter(VAF_f, VAF_cumulative,  s=80,edgecolors='k',linewidths=0.4,alpha=0.6)
    # print('Whole_KS',D_ks,W_Ks,'  Cancer number:',cancer_num)


    plt.figure('Whole cumulative0', figsize=[5, 5])

    plt.xlabel('Inverse allelic frequency 1/f')
    plt.ylabel("Cumulative Number of Mutations M(f)")
    plt.legend(labels='', loc='lower right', shadow=bool, title='Cancer Num= {}'.format(cancer_num), numpoints=None)
    plt.scatter(VAF_f0, VAF_cumulative0, s=80, edgecolors='k', linewidths=0.4, alpha=0.6)
    ...
    pdf = matplotlib.backends.backend_pdf.PdfPages('%s+{}+%s.pdf'.format(cancer_num) % (push_rate, random.randint(1,100)))
    for fig in range(1, 53):  ## will open an empty extra figure :(
        pdf.savefig(fig)
    pdf.close()
